...
// 官网与面板 index.html 自动打缓存戳（app.js?v=<时间戳>），避免浏览器缓存旧包
...
# [官网相关路径已脱敏：官网不开源，内部结构不公开]
  ...
  try {
    ...
    writeFileSync(htmlPath, html.replace(/(app\.js)(\?v=[^"]*)?(")/, `$1?v=${stamp}$3`))
    console.log(`[OK] 缓存戳已更新: ${htmlPath.replace(/\\/g, '/').split('/').slice(-4).join('/')} ?v=${stamp}`)
  } catch {
    console.log(`[!] 缓存戳更新失败: ${htmlPath}`)
  }
}
